...
public class MaximumOfAllSubarraysOfSizeK {
    ...
    static ArrayList<Integer> max_of_subArrays(int arr[], int n, int k) {
        ...
        int pt = 0;
        while(j < n){
            if( dq.isEmpty() || arr[j] < dq.peekLast()  ) {
                dq.addLast(arr[j]);
            }else if( !dq.isEmpty() || arr[j] > dq.peekLast()  ) {
